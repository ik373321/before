class TweetsController < ApplicationController
  ...
  def update
    @tweets = Tweet.find(params[:id])
    if @tweets.update(update_params)
  ...
  def tweets_params
    params.require(:tweets_tag).permit(:title, :risk_id, :country_id, :city, :block, :year, :month, :day_id, :time, :detail,
                                       :name).merge(user_id: current_user.id)
  end

  def update_params
    params.require(:tweet).permit(:title, :risk_id, :country_id, :city, :block, :year, :month, :day_id, :time, :detail,
                                  :name).merge(user_id: current_user.id)
  end
